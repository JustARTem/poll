...
<div class="page">
	...
	<div class="page__content page__content--padding">
		<div class="poll">
			<?php  if ($arPoll['answers']){?>
			<h1 id="question">
				<?php echo $arPoll['question']?>
			</h1>
				...
				<form name="add-vote" method="post" action="">
					...
					<div class="ex2-question__answer">
						...
						foreach ($arPoll['answers'] as $key => $value){
							?>
							<label>
								<input type="radio" name="answer_id" value="<?php echo $key?>">
								<?php echo $value?>
							</label>
						<?php
						}
			...
			<thead>
				...
				<script>
					...
					const voted = false;
					const answers = {
						<?php foreach ($arPoll['answers'] as $key => $value) echo "'$key':'$value',";?>
					};
					let tempresult = {},
						oldresult = {};
						...
						btn.onclick = function (){
							...
							if (!voted){
								if ( !!formData.get('fullname') && parseInt(formData.get('answer_id'))>0){
									...
									formData.append('question_id','<?php echo $questionId?>');
									req.send( formData );
					...
					function getResult(){
						let req = new XMLHttpRequest();
						req.open('GET', 'ajax.php?action=getpollresult&id=<?php echo $questionId?>', true);
